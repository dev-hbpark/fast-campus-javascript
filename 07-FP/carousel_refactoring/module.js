...
overflow: hidden;`;
  ...
  go(
    itemList,
    (srcList) => addImageItems(srcList, itemWidth),
    (container) => {
      wrapper.appendChild(container);
      return container;
    },
    (container) => addButtons(container, itemWidth, slideCount),
    (buttons) => wrapper.append(...buttons)
  );
  return wrapper;
}

function addImageItems(srcList, itemWidth) {
  srcList = srcList || [
    "./images/02.jpg",
    "./images/03.jpg",
    "./images/04.jpg",
...
align-items: center;`; // 인터렉션 예제에서 요거 추가해야 됨
  ...
  return go(
    srcList,
    map((src) => addImageItem(src, itemWidth, "Caption")),
    map((item) => itemContainer.appendChild(item)),
    () => itemContainer
  );
}

function handleSlide(next = true, itemContainer, itemWidth, slideCount = 1) {
  ...
  setTimeout(() => {
    handleTransitionEnd(next, itemContainer, itemWidth, slideCount);
  }, 0);
}

function handleTransitionEnd(next, itemContainer, itemWidth, slideCount = 1) {
...
function addButtons(itemContainer, itemWidth, slideCount) {
...
filter: drop-shadow(3px 5px 2px rgb(0 0 0 / 0.7));`;
  ...
  nextButton.onclick = () => {
    handleSlide(true, itemContainer, itemWidth, slideCount);
  };
  prevButton.onclick = () => {
    handleSlide(false, itemContainer, itemWidth, slideCount);
  };
